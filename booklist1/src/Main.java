...
public class Main {
    public static void main(String[] args) {
        ...
        // 조건 3: 책 제목에 "경제"라는 용어가 들어간 책 제목 조회
        bookList.stream()
        ...
        double maxPrice = bookList.stream()
                ...
                .max()
                .orElse(0.0); // OptionalDouble이 비어있을 경우 기본값 0.0 반환
        System.out.println("책 목록 중 가장 비싼 금액: " + maxPrice);
        System.out.println();

        // 조건 5: 카테고리가 "IT"인 책들의 가격 합 조회
        ...
        // 조건 6: IT 책 할인 이벤트 (새로운 리스트 생성)
        List<Book> discountedBookList = bookList.stream()
                .filter(book -> book.getCategory().equals("IT")) // 카테고리가 IT인 책 필터
                .map(book -> new Book(
                        book.getId(),
                        book.getBookName(),
                        book.getAuthor(),
                        book.getCategory(),
                        book.getPrice() * 0.6)) // 새로운 할인된 책 객체 생성
                .toList();
